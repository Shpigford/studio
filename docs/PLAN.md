...
- [x] **Phase 11: Port Lines** — Largest tool (4750 lines source). 7 shape types (horizontal/vertical/circles/dots/spiral/radial/lissajous). 65+ settings across 15 sidebar sections. Two gradient editors (line color + background) with HSB interpolation and hue wrapping. WEBGL post-processing shader for refraction (6 types with FBM), chromatic aberration, blur, pixelation, halftone (dots/lines/crosshatch), CRT/VHS effects. Watercolor multi-layer rendering with pigment variation and edge darkening. Animation with MP4 recording. Canvas 2D API for hot rendering loops (direct strokeStyle/fillStyle, pre-parsed gradient stops, LRU-1 HSB→RGB cache, precomputed perpendiculars for ribbon mode). Noise buffer uses fast xorshift PRNG instead of p.noise() (100x faster). Conditional noise/freqVar/rotation skipping when disabled. Graceful WebGL fallback. Suspense fallback loading indicator added for all lazy-loaded tools. Done.
